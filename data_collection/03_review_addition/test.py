...
fold_report = metrics.flat_classification_report(y, fold_pred, labels=sorted_labels, digits=3)
print(fold_report)
...
i=0
while (i!=1):
    # f = open("02+03_341dataset_result.txt", "a")
    f = open("final_result.txt", "w")

    # f.write("training data : " + str(file_name) +"\n")
    # f.write(str(e)+"\n")
    # f.write(str(sklearn_crfsuite.metrics.flat_accuracy_score(y_test, y_pred))+'\n')
    # f.write(str(metrics.flat_classification_report(y_test, y_pred, labels=sorted_labels, digits=3)))
    
    f.write("10-Fold Report\n"+str(fold_report)+"\n")
    
    test_data = open("test_dataset.txt", "r").read()
    f.write(str(get_ner(test_data)))
